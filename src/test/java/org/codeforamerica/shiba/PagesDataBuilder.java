package org.codeforamerica.shiba;

import org.codeforamerica.shiba.pages.data.InputData;
import org.codeforamerica.shiba.pages.data.PageData;
import org.codeforamerica.shiba.pages.data.PagesData;

import java.util.List;
import java.util.Map;
import java.util.stream.Collectors;

public class PagesDataBuilder {
    public PagesData build(List<PageDataBuilder> pageDataBuilders) {
        return new PagesData(pageDataBuilders.stream().map(page -> {
            return Map.entry(
                    page.getPageName(),
                    page.getPageDataMap().entrySet().stream().reduce(
                            new PageData(),
                            (pageData, entry) -> {
                                pageData.put(entry.getKey(), InputData.builder().value(entry.getValue()).build());
                                return pageData;
                            },
                            (entry1, entry2) -> {
                                entry1.putAll(entry2);
                                return entry1;
                            }
                    )
            );
//            pagesData.put(
//                    page.getPageName(),
//
//            )
//            page.getPageDataMap().entrySet().stream().map(input -> new PageData(Map.of(
//                    input.getKey(),
//                    InputData.builder().value(List.of(input.getValue())).build()
//            )));
        }).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue)));
    }
}
